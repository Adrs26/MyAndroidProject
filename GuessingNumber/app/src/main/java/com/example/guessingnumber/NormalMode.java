...
public class NormalMode extends AppCompatActivity {
    ...
    private void initComponents() {
        score = findViewById(R.id.normalScore);
        guessChance = findViewById(R.id.normalGuessChance);
        stage = findViewById(R.id.normalStage);
        guess = findViewById(R.id.normalInputLayout).findViewById(R.id.normalInputEditText);
        hint = findViewById(R.id.normalHint);
        guessButton = findViewById(R.id.normalGuessButton);
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AtomicInteger currentGuessChance = new AtomicInteger(7);
        ...
        AtomicInteger currentNumber = new AtomicInteger(NumberGenerator.normalNumberGenerator());
        ...
        guessButton.setOnClickListener(v -> {
            ...
            if (guessString.isEmpty()) {
                Toast.makeText(NormalMode.this, "Harap masukan angka dengan rentang 1 - 150", Toast.LENGTH_LONG).show();
            } else {
                ...
                if (guessNumber == currentNumber.get()) {
                    ...
                    AlertDialog.Builder dialog = new AlertDialog.Builder(NormalMode.this);
                    ...
                    dialog.setPositiveButton("Lanjut", (dialog1, which) -> {
                        ...
                        } else if (currentAttempt.get() == 6) {
                            currentScore.addAndGet(3500);
                        } else if (currentAttempt.get() == 7) {
                            currentScore.addAndGet(3500);
                        }
                        ...
                        if (currentStage.get() > 5) {
                            ...
                            Intent intent = new Intent (NormalMode.this, FinalScore.class);
                            startActivity(intent);
                        } else {
                            currentAttempt.set(0);
                            currentGuessChance.set(7);
                            currentNumber.set(NumberGenerator.normalNumberGenerator());
                            score.setText(String.valueOf(currentScore.get()));
                            guessChance.setText(String.valueOf(currentGuessChance.get()));
                            stage.setText(getResources().getString(R.string.stage, currentStage.get()));
                            guess.setText("");
                ...
                else if (Math.abs(guessNumber - currentNumber.get()) <= 5 && guessNumber - currentNumber.get() < 0 && currentGuessChance.get() > 1) {
                    currentAttempt.getAndIncrement();
                    currentGuessChance.getAndDecrement();
                    guessChance.setText(String.valueOf(currentGuessChance.get()));
                    hint.setText(getResources().getString(R.string.hint1_1));
                    hint.setTextColor(ContextCompat.getColor(NormalMode.this, R.color.black));
